...
import { Switch } from "@/components/ui/switch"; // Added for customizable alerts
import { Label } from "@/components/ui/label"; // Added for customizable alerts
import { Brain, Clock, Volume2, VolumeX, Calendar, Smartphone, Settings2 } from "lucide-react"; // Settings2 added for customization icon
...
export default function AlertSystem({ 
  ...
  const [customAlertConfig, setCustomAlertConfig] = useState(() => {
    try {
      const storedConfig = localStorage.getItem('customMeetingAlertConfig');
      if (storedConfig) {
      ...
      console.error("Failed to parse custom alert config from localStorage", e);
      return { // Fallback defaults in case of error
        minutesBefore: 15,
        soundEnabled: true,
        vibrationEnabled: true,
        speechEnabled: true,
        intensity: 'max' // Default intensity in fallback
      };
    }
  });
  ...
  // Save custom alert config to localStorage whenever it changes
  useEffect(() => {
    try {
      localStorage.setItem('customMeetingAlertConfig', JSON.stringify(customAlertConfig));
    } catch (e) {
      console.error("Failed to save custom alert config to localStorage", e);
    }
  }, [customAlertConfig]);
    ...
    const loadUserPreferences = async () => {
      try {
        ...
        if (prefsList.length > 0) {
          ...
          localStorage.setItem('customMeetingAlertConfig', JSON.stringify(newConfig));
  ...
  const handleSnooze = async (minutes) => {
    ...
    try {
      localStorage.setItem(`meetingAlert_${meeting?.id}`, JSON.stringify({
