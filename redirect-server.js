...
// Store authentication data in memory (in production, use a database)
let authData = null;
  ...
  if (error) {
    ...
    try {
      ...
      if (tokenData.access_token) {
        ...
        console.log('Authentication data stored for app to retrieve');
      ...
      authData = { success: false, error: error.message };
      res.send(`
        <html>
          <head><title>OAuth Error</title></head>
          <body>
    ...
    res.send(`
      <html>
        ...
        <body>
          ...
          <p>This server handles OAuth redirects for development.</p>
...
app.get('/check-auth', (req, res) => {
  ...
  if (authData && authData.success) {
    console.log('Sending successful auth data to app');
    res.json(authData);
    // Clear the data after sending it
    authData = null;
  } else {
    console.log('No auth data available, sending failure response');
    res.json({ success: false, message: 'No authentication data available' });
  }
});
...
app.listen(port, () => {
  ...
  console.log(`Check auth URL: http://localhost:${port}/check-auth`);
  console.log('');
  console.log('To make this public, run: ngrok http 3000');
  console.log('Then add the ngrok URL to Google Cloud Console');
  console.log('');
  console.log('Client detection is now automatic based on User-Agent');
});
