...
async function getTodayUsage(userId) {
  try {
    if (!supabase) {
      ...
      return { total_requests: 0, ai_requests: 0, meeting_requests: 0 };
    }
    ...
    if (error && error.code !== 'PGRST116') { // PGRST116 = no rows returned
      console.warn('Error fetching usage, table may not exist:', error.message);
      return { total_requests: 0, ai_requests: 0, meeting_requests: 0 };
    }

    if (!usage) {
      // Try to create today's usage record
      try {
        ...
        if (createError) {
          console.warn('Error creating usage record:', createError.message);
          return { total_requests: 0, ai_requests: 0, meeting_requests: 0 };
        }
        return newUsage;
      } catch (createErr) {
        console.warn('Error creating usage record:', createErr.message);
        return { total_requests: 0, ai_requests: 0, meeting_requests: 0 };
      }
    }
...
async function incrementUsage(userId, requestType = 'total') {
  try {
    ...
    // Try to use PostgreSQL's increment functionality
    try {
      ...
      if (error) {
        console.warn('RPC increment failed, trying manual increment:', error.message);
        throw error; // Fall through to manual increment
      }
      ...
      try {
        ...
        console.warn('Manual increment also failed:', upsertError.message);
...
async function checkDailyLimit(userId, userPlan) {
  try {
    ...
    console.warn('Daily limit check error:', error.message);
      ...
      try {
        if (supabase) {
          ...
          if (!userError && user) {
            ...
            userEnabled = user.enabled !== false; // Default to true if not set
          }
        }
      } catch (userErr) {
        console.warn('Error fetching user plan, using defaults:', userErr.message);
...
const adminOnly = async (req, res, next) => {
  try {
    ...
    if (!supabase) {
      console.warn('Supabase not configured, allowing admin access');
      return next();
    }
